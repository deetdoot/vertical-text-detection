...
- Crops the detected region using the rotated box and exports it to the output folder ('detected_segments').
...
def detect_and_crop_vertical_text(yolo_weights='runs/obb/train2/weights/last.pt', images_dir='gemini-validation-photos/', output_dir='detected_segments/'):
    model = YOLO(yolo_weights)
    for img_name in image_files:
        ...
        for result in results:
            ...
            for i, name in enumerate(names):
                ...
                # Save cropped image
                crop_filename = f"{output_dir}{os.path.splitext(img_name)[0]}_rotcrop_{i}_{name}.jpg"
                cv2.imwrite(crop_filename, crop_img_masked)


def main(yolo_weights='runs/obb/train2/weights/last.pt', images_dir='gemini-validation-photos/', output_dir='detected_segments/'):
    detect_and_crop_vertical_text(
